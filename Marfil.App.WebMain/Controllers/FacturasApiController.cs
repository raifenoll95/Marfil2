...
{
    public class FacturasApiController : ApiBaseController
    {
        public FacturasApiController(IContextService context) : base(context)
        {
        }
        ...
        public HttpResponseMessage Get()
        {
            ...
            using (var service = FService.Instance.GetService(typeof(FacturasModel),ContextService))
            {
                ...
                {
                    values = service.getAll().Select(f=>(FacturasModel)f),
                    columns = new[]
                    {
