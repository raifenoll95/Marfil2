...
{
    public class CuentasTiposClientesProspectosExclusiveApiController : ApiBaseController
    {
        public CuentasTiposClientesProspectosExclusiveApiController(IContextService context) : base(context)
        {
        }
        ...
        public HttpResponseMessage Get(string id)
        {
            ...
            using (var service = FService.Instance.GetService(typeof(ProspectosModel),ContextService) as ProspectosService)
            {
                var nvc = HttpUtility.ParseQueryString(Request.RequestUri.Query);
                var primeracarga = Funciones.Qbool(nvc[Constantes.Primeracarga]);
                var list = service.GetProspectoCliente(id);
