...
namespace Marfil.Dom.Persistencia.Helpers
{
    internal class ServiceHelper
    {
        ...
        public static string GetNextId<T>(MarfilEntities db, string empresa, string serie, string tipodocumento, TipoInicio tipoinicio = TipoInicio.Sinreinicio, DateTime? fecha = null) where T : class
        {
            string max;
            ...
            var last = Funciones.Qint(max);
            var result = last.HasValue ? (last + 1).ToString() : string.Empty;
            var contador = db.Series.Single(f => f.tipodocumento == tipodocumento && f.empresa == empresa && f.id == serie).fkcontadores;
            var contadorObj = db.Contadores.Include("ContadoresLin").Single(f => f.empresa == empresa && f.id == contador);
            if (string.IsNullOrEmpty(max))
            {
                result = contadorObj.primerdocumento?.ToString() ?? string.Empty;
        ...
        public static string GetReference<T>(MarfilEntities db, string empresa, string fkseries, string tipodocumento, string fkcontadores, DateTime fecha, out string identificador) where T : class
        {
            var sb = new StringBuilder();
            var serie = db.Series.Single(f => f.tipodocumento == tipodocumento && f.empresa == empresa && f.id == fkseries);
            var contador = db.Contadores.Include("ContadoresLin")
                .Single(f => f.empresa == empresa && f.id == serie.fkcontadores);
            identificador = "";
            foreach (var item in contador.ContadoresLin)
            {
                ...
                {
                    identificador = contador.tipoinicio != (int?)TipoInicio.Sinreinicio
                            ? GetNextId<T>(db, empresa, fkseries, tipodocumento, (TipoInicio)contador.tipoinicio, fecha)
                            : fkcontadores;
