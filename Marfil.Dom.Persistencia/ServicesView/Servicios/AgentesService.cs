...
namespace Marfil.Dom.Persistencia.ServicesView.Servicios
{
    ...
    public class AgentesService : GestionService<AgentesModel, Agentes>, IAgentesService
    {
        #region CTR

        public AgentesService(IContextService context, MarfilEntities db = null) : base(context, db)
        {
        ...
        {
            return string.Format(CuentasService.SelectCuentasTerceros, (int)TiposCuentas.Agentes, Empresa, TiposCuentas.Agentes);
        }
        ...
        private void DeleteCuentas(AgentesModel model)
        {
        ...
        private void GuardarCuentas(AgentesModel model)
        {
            model.Cuentas.Tiposcuentas = (int)TiposCuentas.Agentes;
            var fservice = FService.Instance;
        ...
        private int GetNextId(AgentesModel model)
        {
            var result = 1;
        ...
        private void GuardarDirecciones(AgentesModel model)
        {
            ...
            direccionesService.CleanAllDirecciones(Empresa, TiposCuentas.Agentes, model.Fkcuentas);
            foreach (var item in model.Direcciones.Direcciones)
            {
        ...
        private void DeleteDirecciones(AgentesModel model)
        {
            if (model.Direcciones == null) return;
            var fservice = FService.Instance;
        ...
        private void ProcessBancos(AgentesModel model)
        {
        ...
        private string GetNextBancosId(AgentesModel model)
        {
        ...
        private void GuardarBancos(AgentesModel model)
        {
        ...
        private void DeleteBancos(AgentesModel model)
        {
            if (model.BancosMandatos == null) return;
            var fservice = FService.Instance;
        ...
        {
            return _db.Set<Agentes>().ToList().Select(f => _converterModel.GetModelView(f) as AgentesModel);
        }
    }
}
