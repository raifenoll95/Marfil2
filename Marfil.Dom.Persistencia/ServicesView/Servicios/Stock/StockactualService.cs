    ...
    public class StockactualService
    {
            ...
            string familiadesde, string familiahasta, string lotedesde, string lotehasta, bool solotablas, TipoCategoria flujo, string acabadodesde, string acabadohasta)
        {
            var a = string.Format("select s.*, s.metrosentrada as [MetrosEntrada],s.cantidaddisponible as [Cantidad],u.decimalestotales as [Decimalesmedidas],a.descripcionabreviada as [Descripcion],fp.id as [Fkfamilias],concat(bl.fkbundlelote,bl.fkbundle) as [Bundle],REPLACE(STR(s.loteid, 3), SPACE(1), '0') as Loteidentificador   from stockhistorico as s " +
                                 ...
                                 " where isnull(s.lote,'') <> '' and s.empresa='{0}' and s.fkalmacenes='{1}' {2} order by  s.lote asc,REPLACE(STR(s.loteid, 3), SPACE(1), '0') asc", empresa, fkalmacen, CrearFiltrosArticulos(articulodesde, articulohasta, lotedesde, lotehasta, acabadodesde, acabadohasta,false,false), CrearFiltrosFamilias(familiadesde, familiahasta, solotablas));
            return a;
        }
        #endregion
            ...
            string familiadesde, string familiahasta, string lotedesde, string lotehasta, bool solotablas,string lote)
        {
            return string.Format("select top 1 s.*,s.cantidaddisponible as [Cantidad],u.decimalestotales as [Decimalesmedidas],a.descripcionabreviada as [Descripcion],fp.id as [Fkfamilias],b.descripcion as [Bundle]  from stockactual as s " +
                                 ...
                                 " where s.empresa='{0}' and s.fkalmacenes='{1}' and s.lote='{4}' {2}", empresa, fkalmacen, CrearFiltrosArticulos(articulodesde, articulohasta, lotedesde, lotehasta,"","",false,false), CrearFiltrosFamilias(familiadesde, familiahasta, solotablas),lote);
        }
        ...
        public IEnumerable<StockActualVistaModel> GetArticulosLotes(string fkalmacen, string empresa,string articulodesde,string articulohasta,string familiadesde,string familiahasta,string lotedesde,string lotehasta,bool solotablas,TipoCategoria flujo,string acabadodesde="",string acabadohasta="")
        {
            ...
            else if (_db.Bundle.Where(f => f.empresa == _context.Empresa && f.lote + f.id == lotedesde && f.lote + f.id == lotehasta).ToList().Count > 0)
            {
                esbundle = true;
                articulodesde = null;
                articulohasta = null;
            ...
            var sql = GetCadenaBusquedaLotes(fkalmacen, empresa, articulodesde, articulohasta, familiadesde, familiahasta, lotedesde, lotehasta, solotablas, flujo, acabadodesde, acabadohasta, eskit, esbundle);
        ...
        private string GetCadenaBusquedaLotes(string fkalmacen, string empresa, string articulodesde, string articulohasta,
            string familiadesde, string familiahasta, string lotedesde, string lotehasta, bool solotablas, TipoCategoria flujo,string acabadodesde,string acabadohasta, bool eskit, bool esbundle)
        {
            if (eskit)
            {
                return string.Format("select kitlin.*,CAST(kitlin.cantidad as float) as [Cantidad],u.decimalestotales as [Decimalesmedidas],a.descripcionabreviada as [Descripcion],fp.id as [Fkfamilias],kit.referencia as [Bundle],REPLACE(STR(kitlin.loteid, 3), SPACE(1), '0') as Loteidentificador  " +
                                 ...
                                 " where isnull(kit.referencia,'') <> '' and kit.empresa='{0}' and kit.fkalmacen='{1}' {2} order by kitlin.lote asc,REPLACE(STR(kitlin.loteid, 3), SPACE(1), '0') asc", empresa, fkalmacen, CrearFiltrosArticulos(articulodesde, articulohasta, lotedesde, lotehasta, acabadodesde, acabadohasta,eskit,esbundle), CrearFiltrosFamilias(familiadesde, familiahasta, solotablas));
            }
            else if (esbundle)
            {
                return string.Format("select kitlin.*,CAST(kitlin.cantidad as float) as [Cantidad],u.decimalestotales as [Decimalesmedidas],a.descripcionabreviada as [Descripcion],fp.id as [Fkfamilias],Concat(kit.lote,kit.id) as [Bundle],REPLACE(STR(kitlin.loteid, 3), SPACE(1), '0') as Loteidentificador  " +
                                 " from bundle as kit " +
                                 " inner join BundleLin as kitlin on Concat(kit.lote,kit.id) = Concat(kitlin.fkbundlelote,kitlin.fkbundle) " +
                                 " inner join articulos as a on a.empresa= kit.empresa and a.id=kitlin.fkarticulos and (a.categoria=0 or a.categoria=" + (int)flujo + ") " +
                                 " inner join familiasproductos as fp on fp.empresa= kit.empresa and fp.id=substring(kitlin.fkarticulos,0,3) {3} " +
                                 " inner join unidades as u on u.id= kitlin.fkunidades " +
                                 " where isnull(Concat(kit.lote,kit.id),'') <> '' and kit.empresa='{0}' and kit.fkalmacen='{1}' {2} order by kitlin.lote asc,REPLACE(STR(kitlin.loteid, 3), SPACE(1), '0') asc", empresa, fkalmacen, CrearFiltrosArticulos(articulodesde, articulohasta, lotedesde, lotehasta, acabadodesde, acabadohasta, eskit, esbundle), CrearFiltrosFamilias(familiadesde, familiahasta, solotablas));
            }
        ...
        private string CrearFiltrosArticulos(string articulodesde, string articulohasta,string lotedesde,string lotehasta,string acabadodesde,string acabadohasta, bool eskit, bool esbundle)
        {
            ...
            if (!string.IsNullOrEmpty(lotedesde))
            {
                ...
                {
                    sb.AppendFormat(" AND Concat(kit.lote,kit.id)>='{0}' ", lotedesde);
                }
                else
                {
        ...
        //
        public IStockPiezaSingle GetArticuloPorLoteEntradaSingleOCodigo(string referencialote, string articulo, string fkalmacen, string empresa)
        {
            Stockactual pieza = null;
            Kit kitobj = null;
            Bundle bundleobj = null;

            if (!string.IsNullOrEmpty(referencialote) && !string.IsNullOrEmpty(articulo) && _db.Stockactual.Any(f =>
                f.empresa == empresa && f.fkalmacenes == fkalmacen &&
                f.lote == referencialote && f.fkarticulos == articulo))
            {
                pieza = _db.Stockactual.First(f =>
                    f.empresa == empresa && f.fkalmacenes == fkalmacen &&
                    f.lote == referencialote && f.fkarticulos == articulo);
            }
            else if (!string.IsNullOrEmpty(referencialote) && _db.Stockactual.Any(f =>
                f.empresa == empresa && f.fkalmacenes == fkalmacen &&
                f.lote == referencialote))
            {
                pieza = _db.Stockactual.First(f =>
