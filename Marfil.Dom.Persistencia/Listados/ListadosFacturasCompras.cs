...
{
    public class ListadosFacturasCompras : ListadosModel
    {
        private List<string> _series = new List<string>();

        #region Properties

        public override string TituloListado => "Listado de Facturas Compras";

        public override string IdListado
        {
            get { return Agrupacion == "2" ? FListadosModel.FacturasComprasCabeceraAgrupado : FListadosModel.FacturasComprasCabecera; }
        }

        public override string WebIdListado => FListadosModel.FacturasComprasCabecera;
        ...
        public ListadosFacturasCompras()
        {

        }
        
        public ListadosFacturasCompras(IContextService context) : base(context)
        {
        ...
        internal override string GenerarFiltrosColumnas()
        {
            ...
            if (!string.IsNullOrEmpty(Fkmonedas))
            {
                ...
                Condiciones.Add(string.Format("{0}: {1}", RFacturasCompras.Fkmonedas, serviceMonedas.getAll().Select(f => (MonedasModel)f).SingleOrDefault(f => f.Id == int.Parse(Fkmonedas)).Descripcion));
                flag = true;
            }
        ...
        internal override string GenerarSelect()
        {
            ...
            if (Agrupacion == "1")
            {
                sb.Append(
                    ...
                    " inner join cuentas as cu on (c.empresa= cu.empresa and c.fkcuentas=cu.id) or (ac.empresa= cu.empresa and ac.fkcuentas=cu.id)" +
                    " left join direcciones as di on (di.empresa=c.empresa and di.fkentidad=c.fkcuentas) or (di.empresa=ac.empresa and di.fkentidad=ac.fkcuentas) and di.defecto='true' " +
                    ...
                    " left join familiasclientes as fc  on fc.valor=c.fkfamiliaproveedor " +
                    " left join zonasclientes as zc on zc.valor= c.fkzonaproveedor " +
