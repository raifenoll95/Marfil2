...
namespace Marfil.Dom.Persistencia.Listados
{
    public class ListadoPerdidasYGananciasBalanceAnual : ListadosModel
    {
        ...
        internal override string GenerarSelect()
        {
            ...
            if (Desglosarniveltres)
            {
                sb.Append("SELECT cab.orden as Orden, cab.descrip as [Descripción], lin.cuenta as Cuenta, lin.saldo as Saldo FROM ReportGuiasBalancesBalanceAnual cab, ReportGuiasBalancesLineasBalanceAnual lin");
            }
            else
            {
                sb.Append("Select orden as Orden, textogrupo as [Texto Grupo], descrip as [Descripción], saldo as Saldo from ReportGuiasBalancesBalanceAnual");
            }
