...
{
    public class ListadosStockValoradoTarifa : ListadosModel
    {
        ...
        public override string TituloListado => Menuaplicacion.listadostockvaloradotarifa;
        ...
        public override string IdListado
        {
            get
            {
                ...
                {
                    case "0":
                        resultado = FListadosModel.StockGeneralValorado + Detallepor;
                        break;
                    case "1":
                        resultado = FListadosModel.StockAgrupadoArticuloValorado + Detallepor;
                        break;
                    case "2":
                        resultado = FListadosModel.StockAgrupadoArticuloLoteValorado + Detallepor;
                        break;
                    case "3":
                        resultado = FListadosModel.StockAgrupadoArticuloLoteMedidasValorado + Detallepor;
                        break;
                    case "4":
                        resultado = FListadosModel.StockAgrupadoArticuloMedidasValorado + Detallepor;
                        break;
                }
        ...
        #region Properties
        public override string WebIdListado => FListadosModel.StockGeneralValoradoTarifa;
        ...
        [Display(ResourceType = typeof(General), Name = "LblTarifa")]
        public string Tarifa { get; set; }

        #endregion Properties

        public ListadosStockValoradoTarifa()
        {

        }

        public ListadosStockValoradoTarifa(IContextService context) : base(context)
        {
        ...
        internal override string GenerarFiltrosColumnas()
        {
            ...
            if (!string.IsNullOrEmpty(Tarifa))
            {
                ...
                ValoresParametros.Add("fkatarifas", Tarifa);
                sb.Append(" TarLin.fktarifas = @fkatarifas  ");
                Condiciones.Add(string.Format("{0}: {1}", General.LblTarifa, Tarifa));
                flag = true;
            }
        ...
        private string CadenaSinagrupacion()
        {
            ...
            // lo nuevo
            //Precio
            sb.AppendFormat(",ISNULL(TarLin.precio, 0) AS [Precio]");
            //Descuento
            sb.AppendFormat(",ISNULL(TarLin.descuento, 0) AS [% Descuento]");
            // Total
            sb.AppendFormat(",case s.metros");
            sb.AppendFormat(" when 0 then 0 ");
            sb.AppendFormat("  else " +
                "(case ISNULL(TarLin.descuento, 0) when 0 then ISNULL(TarLin.precio, 0) * s.metros  else ((ISNULL(TarLin.precio, 0) * s.metros) - (ISNULL(TarLin.precio, 0) * s.metros) * TarLin.descuento / 100) end)" );
            sb.AppendFormat(" end AS [Total]");            
            ...
            // Tarifa
            sb.AppendFormat(" LEFT JOIN TarifasLin TarLin on TarLin.empresa = s.empresa and TarLin.fkarticulos = s.fkarticulos  ");
            return sb.ToString();
        }
        ...
        private string CadenaAgrupadoArticulos()
        {
            ...
            sb.AppendFormat("ISNULL(TarLin.descuento, 0) AS [% Descuento],");
            // Total
            sb.AppendFormat("SUM(case s.metros ");
            sb.AppendFormat(" when 0 then 0 ");
            sb.AppendFormat("  else " +
                "(case ISNULL(TarLin.descuento, 0) when 0 then ISNULL(TarLin.precio, 0) * s.metros  else ((ISNULL(TarLin.precio, 0) * s.metros) - (ISNULL(TarLin.precio, 0) * s.metros) * TarLin.descuento / 100) end) ");
            sb.AppendFormat(" end) AS [Total] ");
        ...
        {
            return " group by s.fkarticulos, a.descripcion,fp.descripcion,ml.descripcion,fm.descripcion,u.codigounidad,u.decimalestotales,TarLin.precio,TarLin.descuento " +
        ...
        {
            return " group by s.fkarticulos, a.descripcion,fp.descripcion,ml.descripcion,fm.descripcion, s.lote,u.codigounidad,u.decimalestotales,TarLin.precio,TarLin.descuento " +
                        GetColumnasAgrupacionAlmacen() +
                   ", dbo._fn_enum_TipoAlmacenlote(s.tipoalmacenlote)";
        }
        ...
        {
            return " group by s.fkarticulos, a.descripcion,fp.descripcion,ml.descripcion,fm.descripcion, s.lote,s.largo,s.ancho,s.grueso,u.codigounidad,u.decimalestotales,TarLin.precio,TarLin.descuento " +
                        GetColumnasAgrupacionAlmacen() +
                    ", dbo._fn_enum_TipoAlmacenlote(s.tipoalmacenlote)";
        }
        ...
        {
            return " group by s.fkarticulos, a.descripcion,fp.descripcion,ml.descripcion,fm.descripcion, s.largo,s.ancho,s.grueso,u.codigounidad,u.decimalestotales,TarLin.precio,TarLin.descuento " +
                GetColumnasAgrupacionAlmacen() +
                ", dbo._fn_enum_TipoAlmacenlote(s.tipoalmacenlote)";
        }
